...
class SaleOrder(models.Model):
    ...
    def process_contract(self):
        for order in self:
            all_contract_lines = list(filter(lambda line: line.product_id.saas_plan_id and not line.is_processed, order.order_line))
            for contract_line in all_contract_lines:
                _logger.info(" -- -- - - - process contract executed in saas module %r ---"%contract_line._context.get('is_processed'))
                contract_line.is_processed = True   # Need to update
                user_billing = 0
                saas_users = 0
...
class SaasSaleOrderLine(models.Model):
    ...
    is_processed = fields.Boolean(string="Is line Processed", default=False)
